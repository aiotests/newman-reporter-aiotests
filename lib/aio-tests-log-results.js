...
function cleanToMakeLean(summary) {
 ...
 function reportToAIO(aioConfig, o) {
    if(validateConfig(aioConfig)) {
        ...
        return getOrCreateCycle(aioConfig, o.summary.collection.name).then(async (cycleKey) => {
            ...
            bodyFormData.append("updateOnlyRunStatus", aioConfig.updateOnlyRunStatus == null ? 'false' : aioConfig.updateOnlyRunStatus + '');
            bodyFormData.append("defaultFolder", await createOrGetFolder(aioConfig))
...
async function createOrGetFolder(aioconfig) {
    const folderNames = (aioconfig.folderHierarchy || '').split(',');
    const trimmedFolders = folderNames
        .map(name => name.trim())
        .filter(name => name.length > 0);
    if(trimmedFolders.length === 0){
        return "";
    }
    const folderObj= {
        folderHierarchy: trimmedFolders
    };

    const url = "/project/"+ aioconfig.jiraProjectId+ "/testcase/folder/hierarchy"

    return aioAPIClient.put(url, folderObj
    ).then(function (response) {
        aioLogger.log(`Creating new test cases in folder: ${trimmedFolders.join(' > ')} (Folder ID: ${response.data.ID})`);
